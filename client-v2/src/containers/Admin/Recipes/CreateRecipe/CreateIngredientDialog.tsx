...
import { createIngredientAsync } from 'store/recipeEditor/actions';
...
const CreateIngredientDialog: FC<DialogProps> = (props) => {
  ...
  const onButtonClick = () => {
    ...
    if (name && plural) {
      dispatch(createIngredientAsync.request({ name, plural }));
    }
  };
  ...
  return (
    <Dialog {...props}>
      ...
      <DialogContent>
        <DialogContentText>
          Quickly create a new ingredient in the database.
        </DialogContentText>
        <Box my={2}>
          <TextField
            inputRef={nameRef}
